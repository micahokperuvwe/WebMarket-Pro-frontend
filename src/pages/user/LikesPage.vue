  ...
  <div class="min-h-screen bg-primary text-primary">
    ...
    <main class="mx-auto max-w-7xl px-6 py-16">
      ...
      <header class="mb-14 space-y-2">
        ...
        <p class="text-lg font-medium text-white/40">Digital assets you've bookmarked for later.</p>
      </header>

      <!-- Loading state -->
      <div v-if="isLoading" class="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
        <div v-for="i in 3" :key="i" class="glass-panel animate-pulse rounded-[2.5rem] border border-white/5 p-6">
          <div class="mb-6 h-48 rounded-2xl bg-white/5"></div>
          <div class="h-6 w-3/4 rounded-xl bg-white/5 mb-3"></div>
      ...
      <div v-else-if="likedWebsites.length === 0" class="flex flex-col items-center justify-center py-32 text-center">
        ...
        <p class="font-display text-3xl font-black tracking-tight text-white/40">No saved items yet</p>
        <p class="mt-3 text-white/20">Browse our catalog and save the websites you love.</p>
      ...
      <div v-else class="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
        <article
          ...
          :key="site.id"
          class="group glass-panel relative overflow-hidden rounded-[2.5rem] border border-white/5 transition-all duration-500 hover:-translate-y-1 hover:border-white/10 hover:shadow-2xl hover:shadow-gold-500/5"
        >
          <!-- Image -->
          <div class="relative h-52 overflow-hidden">
            ...
            <button
              ...
              class="absolute right-4 top-4 flex h-10 w-10 items-center justify-center rounded-2xl bg-black/40 text-red-400 backdrop-blur-sm transition-all hover:bg-red-500 hover:text-white active:scale-95"
            ...
            <!-- Featured Badge -->
            <div v-if="site.featured" class="absolute left-4 top-4 flex items-center gap-1.5 rounded-xl bg-gold-500/90 px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-black backdrop-blur-sm">
          ...
          <div class="p-6 space-y-4">
            <div>
              ...
              <h2 class="mt-1 font-display text-xl font-extrabold tracking-tight text-white group-hover:text-gold-400 transition-colors">{{ site.title }}</h2>
              <p class="mt-1.5 text-sm text-white/40 line-clamp-2">{{ site.description }}</p>
            </div>
            ...
            <div v-if="site.tech_stack?.length" class="flex flex-wrap gap-2">
              <span
                ...
                :key="tech"
                class="rounded-lg bg-white/5 px-2.5 py-1 text-[10px] font-black uppercase tracking-widest text-white/40"
              >{{ tech }}</span>
            </div>
            ...
            <div class="flex items-center justify-between pt-2 border-t border-white/5">
              <div>
                <p class="text-[10px] font-black uppercase tracking-widest text-white/20">Price</p>
                <p class="font-display text-2xl font-black text-white">{{ formatCurrency(Number(site.price), site.currency) }}</p>
              </div>
              <button
                @click="addToCart(site)"
                class="flex items-center gap-2 rounded-2xl bg-white px-5 py-3 font-display text-xs font-black uppercase tracking-widest text-black transition-all hover:bg-gold-500 active:scale-95"
              >
