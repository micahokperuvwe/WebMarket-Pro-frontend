    ...
    <div class="space-y-10">
      <header class="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
        <div class="space-y-1">
          ...
          <p class="text-lg font-medium text-white/40">Manage your premium digital assets and deployments.</p>
        </div>
        
        <RouterLink 
          to="/admin/websites/new" 
          class="flex items-center gap-2 rounded-2xl bg-white px-8 py-4 font-display text-sm font-black uppercase tracking-widest text-black transition-all hover:bg-gold-500 active:scale-95 shadow-xl shadow-white/5"
        >
      ...
      <section class="flex flex-col gap-4 lg:flex-row lg:items-center">
        <div class="relative flex-1">
          <svg class="absolute left-5 top-1/2 h-5 w-5 -translate-y-1/2 text-white/20" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
          <input 
            v-model="searchQuery"
            type="text" 
            placeholder="Search by title, category, or slug..." 
            class="w-full rounded-2.5xl border border-white/5 bg-white/[0.03] py-5 pl-14 pr-6 text-sm font-medium text-white outline-none transition-all focus:border-gold-500/30 focus:bg-white/[0.05] focus:ring-4 focus:ring-gold-500/5 placeholder:text-white/20"
          />
        </div>
        
        <div class="flex items-center gap-4">
          <select class="h-[60px] rounded-2.5xl border border-white/5 bg-white/[0.03] px-6 text-sm font-black uppercase tracking-widest text-white/60 outline-none transition-all focus:border-gold-500/30">
          ...
          <div class="flex h-[60px] items-center gap-2 rounded-2.5xl bg-white/[0.03] px-6 text-xs font-black uppercase tracking-[0.2em] text-white/30">
             Total: <span class="text-white">{{ pagination.total }}</span>
          </div>
        </div>
      </section>
      ...
      <div class="glass-panel overflow-hidden rounded-[2.5rem] border-white/5 bg-white/[0.02]">
        <div class="overflow-x-auto">
          <table class="w-full text-left">
            <thead>
              <tr class="border-b border-white/5 bg-white/[0.02] text-[11px] font-black uppercase tracking-[0.25em] text-white/30">
            ...
            <tbody class="divide-y divide-white/5">
              <tr v-if="isLoading" v-for="i in 5" :key="i" class="animate-pulse">
                 <td colspan="6" class="px-8 py-10"><div class="h-6 w-full rounded-xl bg-white/5"></div></td>
              </tr>
              
              <tr v-else-if="websites.length === 0">
                 <td colspan="6" class="px-8 py-32 text-center">
                    <div class="inline-flex h-20 w-20 items-center justify-center rounded-3xl bg-white/5 text-white/20 mb-6">
                    ...
                    <p class="font-display text-2xl font-bold tracking-tight text-white/40">No listings found</p>
                    <p class="mt-2 text-white/20 font-medium">Try broadening your search or post a new asset.</p>
                 </td>
              </tr>
              ...
              >
                <td class="px-8 py-6">
                  <div class="flex items-center gap-5">
                    <div class="h-14 w-14 flex-shrink-0 overflow-hidden rounded-2xl bg-white/5 shadow-inner">
                    ...
                    <div>
                      <p class="font-display text-lg font-extrabold text-white group-hover:text-gold-400 transition-colors">{{ site.title }}</p>
                ...
                <td class="px-8 py-6">
                  <span class="inline-flex items-center gap-2 rounded-xl bg-white/5 px-4 py-2 text-[10px] font-black uppercase tracking-widest text-white/60">
                ...
                <td class="px-8 py-6">
                   <div class="space-y-0.5">
                      <p class="font-display text-[10px] font-black uppercase tracking-widest text-white/20">Market Value</p>
                      <p class="font-display text-xl font-black text-white selection:bg-gold-500/30">
                ...
                <td class="px-8 py-6">
                   <div class="flex items-center gap-3">
                      <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-white/5 text-mint-400">
                      ...
                      <div>
                         <p class="text-sm font-bold text-white">{{ site.sales || 0 }}</p>
                         <p class="text-[10px] font-black uppercase tracking-widest text-white/20">Acquisitions</p>
                      </div>
                   </div>
                </td>
                <td class="px-8 py-6">
                  ...
                  <div v-else class="inline-flex items-center gap-2 rounded-xl bg-white/5 px-4 py-2 text-[10px] font-black uppercase tracking-widest text-white/30">
                    Standard
                  </div>
                </td>
                <td class="px-8 py-6 text-right">
                  <div class="flex items-center justify-end gap-3 opacity-0 transition-all duration-300 group-hover:translate-x-0 group-hover:opacity-100">
                    <button class="flex h-10 w-10 items-center justify-center rounded-xl bg-white/5 text-white/60 transition-all hover:bg-white/10 hover:text-white" title="Edit Listing">
                    ...
                    <button @click="deleteWebsite(site.id)" class="flex h-10 w-10 items-center justify-center rounded-xl bg-red-500/10 text-red-400 transition-all hover:bg-red-500 hover:text-white" title="Remove Listing">
        ...
        <!-- Luxury Pagination -->
        <footer class="flex flex-col items-center justify-between gap-6 border-t border-white/5 bg-white/[0.01] px-8 py-6 md:flex-row">
          <p class="font-display text-[10px] font-black uppercase tracking-[0.2em] text-white/20">Showing {{ websites.length }} of {{ pagination.total }} Premium Assets</p>
          <div class="flex items-center gap-4">
            <button 
              @click="pagination.page--" 
              :disabled="pagination.page === 1"
              class="group flex h-12 w-12 items-center justify-center rounded-2xl border border-white/5 bg-white/5 transition-all hover:bg-white/10 disabled:opacity-20"
            >
            ...
            <div class="flex h-12 min-w-[100px] items-center justify-center rounded-2xl bg-white/5 px-6 font-display text-sm font-black text-white">
               {{ pagination.page }} <span class="mx-2 text-white/20">/</span> {{ pagination.totalPages }}
            </div>
            <button 
               @click="pagination.page++" 
