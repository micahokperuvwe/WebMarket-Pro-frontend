  ...
  <AdminShell>
    <div class="mx-auto max-w-5xl space-y-12 pb-20">
      <header class="flex flex-col gap-2">
        ...
        <p class="text-lg font-medium text-white/40">Configure your new website for the global marketplace inventory.</p>
      </header>
      ...
      <form @submit.prevent="onSubmit" class="space-y-10">
        ...
        <section class="glass-panel group overflow-hidden rounded-[3rem] border-white/5 bg-white/[0.02] p-8 transition-all hover:bg-white/[0.03] lg:p-12">
          <div class="flex items-center gap-4 mb-10">
            ...
            <div>
              <h2 class="font-display text-2xl font-black text-white">Identity & Core</h2>
              <p class="text-sm font-bold text-white/20">Define how the world sees your asset.</p>
            </div>
          </div>

          <div class="grid gap-8 sm:grid-cols-2">
            <div class="space-y-3">
              <label class="font-display text-[11px] font-black uppercase tracking-[0.2em] text-white/30">Asset Title</label>
              <input v-model="form.title" type="text" required class="form-input" placeholder="e.g., Ultra SaaS Dashboard" />
            </div>
            <div class="space-y-3">
              <label class="font-display text-[11px] font-black uppercase tracking-[0.2em] text-white/30">Slug (URL Permanent Link)</label>
              <div class="relative">
                 <span class="absolute left-5 top-1/2 -translate-y-1/2 text-white/20 font-bold">/</span>
          ...
          <div class="grid gap-8 sm:grid-cols-4 mt-8">
             <div class="space-y-3">
              <label class="font-display text-[11px] font-black uppercase tracking-[0.2em] text-white/30">Domain / Category</label>
              <select v-model="form.category" class="form-input appearance-none bg-ink-900 shadow-inner">
            ...
            <div class="space-y-3">
              <label class="font-display text-[11px] font-black uppercase tracking-[0.2em] text-white/30">Currency</label>
              <select v-model="form.currency" class="form-input appearance-none bg-ink-900 shadow-inner">
            ...
            <div class="space-y-3">
              <label class="font-display text-[11px] font-black uppercase tracking-[0.2em] text-white/30">Investment</label>
              <input v-model.number="form.price" type="number" step="0.01" required class="form-input" />
            </div>
             <div class="space-y-3">
              <label class="font-display text-[11px] font-black uppercase tracking-[0.2em] text-white/30">Delivery Logic</label>
              <select v-model="form.accessModel" class="form-input appearance-none bg-ink-900 shadow-inner">
          ...
          <div class="space-y-3 mt-8">
            <label class="font-display text-[11px] font-black uppercase tracking-[0.2em] text-white/30">Premium Description (Markdown Support)</label>
        ...
        <section class="glass-panel group overflow-hidden rounded-[3rem] border-white/5 bg-white/[0.02] p-8 transition-all hover:bg-white/[0.03] lg:p-12">
           <div class="flex items-center gap-4 mb-10">
            ...
            <div>
              <h2 class="font-display text-2xl font-black text-white">Engineering & Access</h2>
              <p class="text-sm font-bold text-white/20">The technical backbone of your digital asset.</p>
            </div>
          </div>

          <div class="space-y-8">
             <div class="space-y-4">
              <label class="font-display text-[11px] font-black uppercase tracking-[0.2em] text-white/30">Tech Stack Architecture</label>
              <div class="flex gap-3">
                ...
                <button type="button" @click="addTech" class="rounded-2xl bg-white/5 px-6 font-display text-lg font-black text-white transition-all hover:bg-white hover:text-black">+</button>
              </div>
              <div class="flex flex-wrap gap-2 pt-2">
                <span v-for="t in form.techStack" :key="t" class="inline-flex items-center gap-2 rounded-xl border border-mint-500/20 bg-mint-500/10 px-4 py-2 text-[10px] font-black uppercase tracking-widest text-mint-400">
                  {{ t }}
                  <button type="button" @click="removeTech(t)" class="text-white/20 hover:text-white transition-colors">
            ...
            <div class="grid gap-8 sm:grid-cols-2">
               <div class="space-y-3">
                  <label class="font-display text-[11px] font-black uppercase tracking-[0.2em] text-white/30">Demo / Preview (URL or World Name)</label>
                  <input v-model="form.demoUrl" type="text" required class="form-input" placeholder="e.g., https://demo.mysite.com or My World Name" />
               </div>
               <div class="space-y-3">
                  <label class="font-display text-[11px] font-black uppercase tracking-[0.2em] text-white/30">Secure Source Artifact (.ZIP)</label>
                  <div class="relative cursor-pointer">
                     ...
                     <div class="flex items-center gap-4 rounded-2xl border border-dashed border-white/20 bg-white/5 px-6 py-4 transition-all hover:bg-white/10">
                        <svg class="h-6 w-6 text-white/40" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M9 19l3 3m0 0l3-3m-3 3V10"/></svg>
                        <span class="text-sm font-bold text-white/60">{{ fileStates.sourceFile ? fileStates.sourceFile.name : 'Choose secure ZIP bundle...' }}</span>
                     </div>
                  </div>
               </div>
            </div>
        ...
        <section class="glass-panel group overflow-hidden rounded-[3rem] border-white/5 bg-white/[0.02] p-8 transition-all hover:bg-white/[0.03] lg:p-12">
           <div class="flex items-center gap-4 mb-10">
            ...
            <div>
              <h2 class="font-display text-2xl font-black text-white">Visual Showcase</h2>
              <p class="text-sm font-bold text-white/20">The cinematic representation of your product.</p>
            </div>
          </div>

          <div class="grid gap-10 sm:grid-cols-2">
            <div class="space-y-4">
              <label class="font-display text-[11px] font-black uppercase tracking-[0.2em] text-white/30">Primary Cinema Banner</label>
              <div class="group relative aspect-video cursor-pointer overflow-hidden rounded-[2rem] border-2 border-dashed border-white/5 bg-white/5 transition-all hover:border-gold-500/30 hover:bg-white/10">
                 ...
                 <div v-if="!fileStates.mainImage" class="flex h-full w-full flex-col items-center justify-center gap-4 text-center">
                    <div class="h-12 w-12 rounded-full bg-white/5 flex items-center justify-center text-white/20"><svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M12 4v16m8-8H4"/></svg></div>
                    <p class="text-xs font-black uppercase tracking-widest text-white/20 px-8">Upload main display image</p>
                 </div>
            ...
            <div class="space-y-4">
              <label class="font-display text-[11px] font-black uppercase tracking-[0.2em] text-white/30">Gallery Vault (Multiple)</label>
               <div class="group relative aspect-video cursor-pointer overflow-hidden rounded-[2rem] border-2 border-dashed border-white/5 bg-white/5 transition-all hover:border-mint-500/30 hover:bg-white/10">
                 ...
                 <div v-if="fileStates.gallery.length === 0" class="flex h-full w-full flex-col items-center justify-center gap-4 text-center">
                    <div class="h-12 w-12 rounded-full bg-white/5 flex items-center justify-center text-white/20"><svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path d="M12 4v16m8-8H4"/></svg></div>
                    <p class="text-xs font-black uppercase tracking-widest text-white/20 px-8">Upload feature screenshots</p>
                 </div>
                 <div v-else class="h-full w-full">
            ...
            <div class="space-y-4 sm:col-span-2">
              <label class="font-display text-[11px] font-black uppercase tracking-[0.2em] text-white/30">Commercial Reel (Video Optional)</label>
              <input type="file" @change="(e: any) => fileStates.video = e.target.files[0]" class="form-input px-5" accept="video/*" />
            </div>
          </div>

          <div class="mt-10 flex items-center gap-4 rounded-3xl bg-white/5 p-6 border border-white/5">
            <input type="checkbox" v-model="form.featured" id="featured" class="h-6 w-6 rounded-lg border-white/10 bg-white/10 text-gold-500 focus:ring-gold-400" />
            <div class="flex-1">
               <label for="featured" class="font-display text-base font-black text-white">Feature in Elite Selection</label>
               <p class="text-xs font-bold text-white/30">Promote this asset to the homepage "Featured Inventory" section.</p>
            </div>
          </div>
        </section>
        ...
        <button 
          type="submit" 
          :disabled="isLoading"
          class="group relative flex w-full items-center justify-center gap-4 overflow-hidden rounded-[2.5rem] bg-white px-10 py-8 font-display text-lg font-black uppercase tracking-[0.25em] text-black transition-all hover:bg-gold-500 hover:scale-[1.01] active:scale-95 disabled:opacity-50"
        >
...
.form-input {
  @apply w-full rounded-2xl border border-white/5 bg-white/[0.03] px-6 py-5 text-sm font-bold text-white outline-none transition-all placeholder:text-white/20 focus:border-gold-500/30 focus:shadow-[0_0_20px_rgba(239,185,53,0.05)];
}
</style>
