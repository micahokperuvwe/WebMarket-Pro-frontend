    ...
    <div class="mx-auto flex max-w-[var(--container-max)] items-center justify-between rounded-[2.5rem] border border-primary/5 bg-primary/5 px-6 py-3.5 shadow-2xl backdrop-blur-2xl transition-all duration-500 hover:bg-primary/10 hover:shadow-gold-500/5 lg:px-10 lg:py-4">
      ...
      <RouterLink to="/" class="group flex items-center gap-4 transition-transform duration-300 hover:scale-105 active:scale-95">
        <div class="relative flex h-12 w-12 items-center justify-center overflow-hidden rounded-2xl bg-gradient-premium shadow-xl shadow-gold-500/20">
           <span class="relative z-10 font-display text-xl font-black tracking-tighter text-canvas">W</span>
      ...
      <nav class="hidden items-center gap-2 md:flex">
        <RouterLink
          ...
          :class="route.path === item.to ? 'text-primary' : 'text-secondary hover:text-primary'"
        >
          {{ item.label }}
          <!-- Underline Indicator -->
          <div 
      ...
      <div class="flex items-center gap-4">
        ...
        <button
          ...
          class="group relative flex h-12 w-12 items-center justify-center rounded-2xl border border-primary/5 bg-primary/5 transition-all duration-300 hover:bg-primary/10 active:scale-90"
          title="Switch Mood"
        >
          <div class="transition-transform duration-500 group-hover:rotate-12">
            ...
            <svg v-else xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-primary" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
          </div>
        </button>
        ...
        >
          ...
          <span 
            ...
            class="absolute -right-2 -top-2 flex h-6 w-6 scale-110 items-center justify-center rounded-full bg-gold-500 font-display text-[11px] font-black text-canvas shadow-lg shadow-gold-500/30 ring-4 ring-canvas transition-transform group-hover:scale-125"
          >
        ...
        <div v-if="authStore.currentUser" class="flex items-center gap-2">
          <RouterLink
            to="/settings"
            class="flex items-center gap-3 rounded-2.5xl bg-primary px-5 py-3 text-sm font-black uppercase tracking-widest text-canvas shadow-xl shadow-primary/5 transition-all duration-300 hover:bg-gold-500 active:scale-95"
          >
          ...
          <button 
            @click="handleLogout"
            class="group flex h-12 w-12 items-center justify-center rounded-2xl border border-primary/5 bg-primary/5 transition-all duration-300 hover:bg-red-500/10 hover:text-red-400 active:scale-90"
            title="Sign Out"
          >
        ...
        <RouterLink 
          v-else 
          to="/auth" 
          class="rounded-full bg-primary px-7 py-3 font-display text-sm font-black uppercase tracking-widest text-canvas shadow-xl shadow-primary/5 transition-all duration-300 hover:bg-gold-500 active:scale-95"
        >
          Join Now
        </RouterLink>
      </div>
