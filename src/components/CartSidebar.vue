...
<template>
  <div class="relative z-[100]" aria-labelledby="slide-over-title" role="dialog" aria-modal="true" :class="{ 'pointer-events-none': !cartStore.isSidebarOpen }">
    <!-- Backdrop overlay -->
    <div 
      class="fixed inset-0 bg-canvas/50 backdrop-blur-sm transition-opacity duration-500"
    ...
    <div class="fixed inset-0 overflow-hidden pointer-events-none">
      <div class="absolute inset-0 overflow-hidden">
        <div class="pointer-events-none fixed inset-y-0 left-0 flex max-w-full pr-10">
          ...
          >
            <div class="flex h-full flex-col overflow-y-scroll bg-canvas border-r border-primary/10 shadow-2xl">
              <!-- Header -->
              <div class="flex items-center justify-between px-6 py-6 border-b border-primary/10">
                <h2 class="text-xl font-display font-black text-primary" id="slide-over-title">Your Cart</h2>
                <button
                  type="button"
                  class="relative -m-2 p-2 text-primary/50 hover:text-primary transition-colors"
                  @click="cartStore.isSidebarOpen = false"
                >
              ...
              <div class="flex-1 overflow-y-auto px-6 py-6 sm:px-6">
                <div v-if="isCartEmpty" class="flex flex-col items-center justify-center h-full text-center">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 text-primary/20 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z" />
                  </svg>
                  <p class="text-lg font-medium text-secondary">Your cart is empty</p>
                  <button 
                    @click="cartStore.isSidebarOpen = false"
                ...
                <ul v-else role="list" class="-my-6 divide-y divide-primary/10">
                  <li v-for="item in cartStore.detailedItems" :key="item?.listingId" class="flex py-6">
                    <div class="h-24 w-24 flex-shrink-0 overflow-hidden rounded-xl border border-primary/10 bg-primary/5">
                    ...
                    <div class="ml-4 flex flex-1 flex-col">
                      <div>
                        ...
                        <p class="mt-1 text-sm text-secondary">{{ item?.listing?.category }}</p>
                      </div>
                      <div class="flex flex-1 items-end justify-between text-sm">
                        <p class="text-secondary">Qty {{ item?.quantity }}</p>
              ...
              <div v-if="!isCartEmpty" class="border-t border-primary/10 px-6 py-6 sm:px-6">
                <div class="flex justify-between text-lg font-bold text-primary mb-4">
                  <p>Subtotal</p>
                ...
                <div class="mt-6">
                  <button
                    ...
                    class="flex w-full items-center justify-center rounded-2xl bg-gradient-premium px-6 py-4 font-display text-sm font-black uppercase tracking-[0.2em] text-canvas shadow-xl shadow-gold-500/10 transition-all hover:scale-[1.02] active:scale-95"
                  >
                    Proceed to Checkout
                  </button>
                </div>
                <div class="mt-6 flex justify-center text-center text-sm text-secondary">
                  <p>
                    or
