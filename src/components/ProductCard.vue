...
<template>
  <article class="group relative flex flex-col overflow-hidden rounded-[2.5rem] border border-[var(--border-color)] bg-[var(--bg-secondary)] transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-gold-500/10">
    <!-- Image Section -->
    ...
    <div class="flex flex-1 flex-col p-8 lg:p-10">
      <div class="mb-6 space-y-3">
        <h3 class="font-display text-3xl font-black tracking-tight text-[var(--text-primary)] group-hover:text-gold-400 transition-colors">{{ listing.title }}</h3>
        <p class="line-clamp-2 text-base font-medium leading-relaxed text-[var(--text-secondary)]">{{ listing.description }}</p>
      </div>
      ...
      <div class="mb-8 flex flex-wrap gap-2">
        <span
          ...
          :key="item"
          class="rounded-lg bg-[var(--bg-primary)] px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-[var(--text-secondary)] border border-[var(--border-color)]"
        >
          {{ item }}
        </span>
      </div>
      ...
      <!-- Footer Info -->
      <div class="mt-auto flex items-end justify-between border-t border-[var(--border-color)] pt-8">
        <div class="space-y-1">
          <p class="font-display text-[10px] font-black uppercase tracking-[0.2em] text-[var(--text-secondary)]">Investment</p>
          <p class="font-display text-3xl font-black text-[var(--text-primary)] selection:bg-gold-500/30">{{ formatCurrency(listing.price, listing.currency) }}</p>
        </div>
        
        <RouterLink 
          :to="`/products/${listing.slug}`" 
          class="flex h-14 items-center gap-3 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-primary)] px-8 font-display text-sm font-black uppercase tracking-widest text-[var(--text-primary)] transition-all hover:bg-[var(--text-primary)] hover:text-[var(--bg-primary)]"
        >
          Details
